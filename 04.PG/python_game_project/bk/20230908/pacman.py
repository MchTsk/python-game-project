...
def main():
    ...
    while True:
        ...
        if C.idx == 0:
            ...
            # タイトル画面にタイトルを描画
            draw.draw_text(screen, "PAC-MAN", (C.SCREEN_SIZE+300)/2, C.SCREEN_SIZE/3, 150, BLACK, True)
            ...
            for plc in range(len(C.plt_col)):
                ...
                screen.blit(img_plt, [C.SCREEN_SIZE-600+100*plc, 360])
            
            # タイトル画面にキー案内を描画
            draw.draw_text(screen, "PUSH  [ SPACE ]  TO  START", (C.SCREEN_SIZE+300)/2, C.SCREEN_SIZE/1.2, 100, BLACK, True)
        ...
        # pause画面
            ...
            # pause画面にキー案内を描画
            draw.draw_text(screen, "PUSH  [ SPACE ]  TO  END", (C.SCREEN_SIZE+300)/2, C.SCREEN_SIZE/1.2, 100, BLACK, True)
        
        # 全コースクリア
        elif C.idx == 3:
            tmr_sec = math.floor(C.tmr_all / C.FPS)
            tmr_clear = datetime.timedelta(seconds = tmr_sec)
            draw.draw_text(screen, "GAME  CLEAR!!!", (C.SCREEN_SIZE+300)/2, C.SCREEN_SIZE/3, 100, BLACK, True)
            draw.draw_text(screen, "CLEAR TIME : " + str(tmr_clear), (C.SCREEN_SIZE+300)/2, C.SCREEN_SIZE/2, 100, BLACK, True)
            draw.draw_text(screen, "GET COIN POINT : " + str(C.point), (C.SCREEN_SIZE+300)/2, C.SCREEN_SIZE/1.5, 100, BLACK, True)
            # 7秒後にタイトル画面へ
            if C.tmr == C.FPS * 7:
                C.idx = 0
                C.tmr = 0
        ...
        # ゲームオーバー
            ...
            draw.draw_text(screen, "GAME  OVER", (C.SCREEN_SIZE+300)/2, C.SCREEN_SIZE/2, 100, RED, True)
            # 3秒後にタイトル画面へ
            if C.tmr == C.FPS * 3:
                C.idx = 0
                C.tmr = 0

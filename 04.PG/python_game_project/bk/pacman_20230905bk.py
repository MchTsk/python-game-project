...
# ******************** メインループ ********************
def main():
    global idx, tmr, course, tmr_all, point
    ...
    while True:
        ...
        if idx == 0:
            ...
            if tmr == 1 or tmr % FPS == 0:
                ...
                plt_col = random.sample(img_pl_list, 6)
            ...
            # タイトル画面にタイトルを描画
            draw_text(screen, "PAC-MAN", (SCREEN_SIZE+300)/2, SCREEN_SIZE/3, 150, BLACK, True)

            # タイトル画面にパックマンを描画
            for plc in range(len(plt_col)):
                if plt_col[plc] % 2 == 1:
                    plt_num = plt_col[plc] - 1
                else:
                    plt_num = plt_col[plc]
                img_plt = pygame.transform.rotozoom(img_player[plt_num+plm_tmr%2], -270, 1.7)
                screen.blit(img_plt, [SCREEN_SIZE-600+100*plc, 360])
            
            # タイトル画面にキー案内を描画
            draw_text(screen, "PUSH  [ SPACE ]  TO  START", (SCREEN_SIZE+300)/2, SCREEN_SIZE/1.2, 100, BLACK, True)
        ...
        # ゲームプレイ
            ...
            # プレイ中
                ...
                chk_and_gen_enemy()   # 生成判定（エネミー） ⇒ 生成
                chk_and_gen_goal()    # 生成判定（ゴール） ⇒ 生成
                chk_and_gen_item()    # 生成判定（アイテム） ⇒ 生成
                ...
                # 制限時間に達するとゲームオーバー
                tmr_sec = math.floor(tmr / FPS)
                tmr_now = datetime.timedelta(seconds = tmr_sec)
                if course <= 10 and tmr_now >= time_limit_1_10:
                    idx = 2
                    tmr = 0
                elif 11 <= course <= 20 and tmr_now >= time_limit_11_20:
                    idx = 2
                    tmr = 0
                elif 21 <= course <= 25 and tmr_now >= time_limit_21_25:
                    idx = 2
                    tmr = 0
                elif 26 <= course <= 29 and tmr_now >= time_limit_26_29:
                    idx = 2
                    tmr = 0
                elif course >= 30 and tmr_now >= time_limit_30:
                    idx = 2
                    tmr = 0
        ...
        # ゲームオーバー
            ...
            tmr_sec = math.floor(tmr_all / FPS)
            tmr_clear = datetime.timedelta(seconds = tmr_sec)
            draw_text(screen, "GAME  CLEAR!!!", (SCREEN_SIZE+300)/2, SCREEN_SIZE/3, 100, BLACK, True)
            draw_text(screen, "CLEAR TIME : " + str(tmr_clear), (SCREEN_SIZE+300)/2, SCREEN_SIZE/2, 100, BLACK, True)
            draw_text(screen, "GET COIN POINT : " + str(point), (SCREEN_SIZE+300)/2, SCREEN_SIZE/1.5, 100, BLACK, True)
            # 5秒後にタイトル画面へ
            if tmr == FPS * 5:
                idx = 0
                tmr = 0
...
def draw_maze(sc):
    ...
    for y in range(-7, 8):
        for x in range(-7, 8):
            ...
            my = pl_y + y
            # if x == -7 and y == -7:
            #     print('mx：' + str(mx) + '、my：' + str(my) + '、maze[my][mx]：' + str(maze[my][mx]))
            ...
            if x == 0 and y == 0:
                if pl_muteki%2 == 0:
                    ...
                    img_rz = pygame.transform.rotozoom(img_player[pl_col*2+plm_tmr%2], pl_d*(-90), 1.0)
                    # アイテム使用中 + 使用時間切れ間近
    ...
    if 21 <= course <= 25:
        if pl_scope == 0:
            draw_img(sc, img_scope[pl_scope+1], SCREEN_SIZE/2, SCREEN_SIZE/2)
    elif course >= 26:
        draw_img(sc, img_scope[pl_scope], SCREEN_SIZE/2, SCREEN_SIZE/2)

    # 枠組み
    # pygame.draw.rect(sc, WHITE, [SCREEN_SIZE+30, 30, 240, 340])
    pygame.draw.rect(sc, WHITE, [SCREEN_SIZE+30, 30, 240, 310])
    # pygame.draw.rect(sc, WHITE, [SCREEN_SIZE+30, 400, 240, 290])
    pygame.draw.rect(sc, WHITE, [SCREEN_SIZE+30, 370, 240, 260])
    # pygame.draw.rect(sc, WHITE, [SCREEN_SIZE+30, 720, 240, 150])
    pygame.draw.rect(sc, WHITE, [SCREEN_SIZE+30, 660, 240, 205])
    ...
    for i in range(6):
        ...
        sc.blit(img_rz, [SCREEN_SIZE+70, 35+50*i])
        # 文字の描画
        # draw_text(sc, "X   " + str(count_enemy_color[i]), SCREEN_SIZE+150, 60+50*i, 35, BLACK, False)
        draw_text(sc, "X   " + str(count_enemy_color[i]), SCREEN_SIZE+150, 45+50*i, 35, BLACK, False)
    ...
    for i in range(1, 6):
        ...
        sc.blit(img_rz, [SCREEN_SIZE+110, 325+50*i])
        # 文字の描画
        # draw_text(sc, "[" + str(i) + "]:", SCREEN_SIZE+50, 380+50*i, 35, BLACK, False)
        draw_text(sc, "[" + str(i) + "]:", SCREEN_SIZE+50, 335+50*i, 35, BLACK, False)
        # draw_text(sc, "X   " + str(pl_item[i]), SCREEN_SIZE+180, 380+50*i, 35, BLACK, False)
        draw_text(sc, "X   " + str(pl_item[i]), SCREEN_SIZE+180, 335+50*i, 35, BLACK, False)
    
    # COIN：獲得コイン数 / ライフ増加のための必要コイン数
    if course <= 5:
        str_pll_inc_coin = str(pll_inc_coin_1)
    elif 6 <= course <= 10:
        str_pll_inc_coin = str(pll_inc_coin_2)
    elif 11 <= course <= 20:
        str_pll_inc_coin = str(pll_inc_coin_3)
    elif course >= 21:
        str_pll_inc_coin = str(pll_inc_coin_4)
    ...
    # 各コース（レベル）の制限時間
    if course <= 10:
        str_time_limit = str(time_limit_1_10)[2:]
    elif 11 <= course <= 20:
        str_time_limit = str(time_limit_11_20)[2:]
    elif 21 <= course <= 25:
        str_time_limit = str(time_limit_21_25)[2:]
    elif 26 <= course <= 29:
        str_time_limit = str(time_limit_26_29)[2:]
    elif course >= 30:
        str_time_limit = str(time_limit_30)[2:]

    # プレイヤーの情報
    draw_text(sc, "LEVEL    :   " + str(course), SCREEN_SIZE+40, 680, 35, BLACK, False)
    draw_text(sc, "TIME       :   " + str_tmr, SCREEN_SIZE+40, 715, 35, BLACK, False)
    draw_text(sc, "LIMIT      :   " + str_time_limit, SCREEN_SIZE+40, 750, 35, BLACK, False)
    draw_text(sc, "LIFE        :   " + str(pl_life), SCREEN_SIZE+40, 785, 35, BLACK, False)
    draw_text(sc, "COIN       :   " + str(pl_coin) + " / " + str_pll_inc_coin, SCREEN_SIZE+40, 820, 35, BLACK, False)
    # draw_text(sc, "POINT      :   " + str(point), SCREEN_SIZE+40, 850, 35, BLACK, False)

    # if tmr==5:
    #     messagebox.askokcancel('maze', maze)
...
def init_maze():
    ...
    for y in range(maze_num):
        # 配列の中にmaze_num分の「0」の配列を作成。for文でmaze_num分作成。各マスの情報が格納される
...
def move_enemy():
    for n in range(emy_max):
        ...
        # エネミーの色：青、赤、黄、緑、茶 -> 移動方向：目標へ移動(追尾)
        else:
            # 青、赤：プレイヤーを追尾
            ...
            # 緑：コインへ移動 or プレイヤーを追尾（フィールド上の全コインがなくなり次第追従）
...
def use_item(key):
    ...
    # 赤色のパックマン（壁破壊）
    if course >= 1 and key[pygame.K_1] == True and pl_item[COLOR_RED] > 0 and pl_col != COLOR_RED:
        snd_pacman_red.play()
        item_effect_off()
        pl_item[COLOR_RED] -= 1
        pl_col = COLOR_RED
        ...
        item_time = FPS * 8
    # 青色のパックマン（無敵）
    if course >= 10 and key[pygame.K_2] == True and pl_item[COLOR_BLUE] > 0 and pl_col != COLOR_BLUE:
        ...
        item_time = FPS * 18
    # 緑色のパックマン（ゴール矢印）
    if course >= 15 and key[pygame.K_3] == True and pl_item[COLOR_GREEN] > 0 and pl_col != COLOR_GREEN:
        ...
        item_time = FPS * 23
    # 黄色のパックマン（描画範囲拡大）
    if course >= 20 and key[pygame.K_4] == True and pl_item[COLOR_YELLOW] > 0 and pl_col != COLOR_YELLOW:
        ...
        item_time = FPS * 17
    # 茶色のパックマン（全体スタン）
    if course >= 25 and key[pygame.K_5] == True and pl_item[COLOR_BROWN] > 0 and pl_col != COLOR_BROWN:
...
def set_target(target):
    ...
    while True:
        ...
        if maze[y][x] == ROAD or maze[y][x] == COIN:
            # プレイヤーと離れている -> ok
            if (x < pl_x-dis or pl_x+dis < x) and (y < pl_y-dis or pl_y+dis < y):
                maze[y][x] = target
                break
...
def hit_check():
    ...
    global pll_inc_coin_1, pll_inc_coin_2, pll_inc_coin_3, pll_inc_coin_4
    ...
    if maze[pl_y][pl_x] == COIN:
        ...
        if course <= 5:
            if pl_coin >= pll_inc_coin_1:
                pl_life += 1
                pl_coin -= pll_inc_coin_1
        elif 6 <= course <= 10:
            if pl_coin >= pll_inc_coin_2:
                pl_life += 1
                pl_coin -= pll_inc_coin_2
        elif 11 <= course <= 15:
            if pl_coin >= pll_inc_coin_3:
                pl_life += 1
                pl_coin -= pll_inc_coin_3
        elif course >= 16:
            if pl_coin >= pll_inc_coin_4:
                pl_life += 1
                pl_coin -= pll_inc_coin_4
    ...
    for n in range(emy_max):
        ...
        if emy_col[n] == COLOR_YELLOW and maze[emy_y[n]][emy_x[n]] == GOAL:
            ...
            goal_generate_time = FPS * 5       # ゴールを再生成するまでの時間
...
def init_game_place():
    ...
    # エネミー
    emy_max = maze_num // 5     # エネミーの最大数（maze_num：初期は17+1）
    emy_num_max = False         # エネミーが最大数いるか(初期時はいない)
    emy_time = FPS * 20         # エネミーを再生成するまでの時間
    init_enemy()                # エネミーの初期化

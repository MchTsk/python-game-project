...
def bring_enemy():
    ...
    # エネミーの配置：ランダム選択
    while True:
        emy_x = random.randint(1, C.maze_num-2)
        emy_y = random.randint(1, C.maze_num-2)
        # 位置が通路(コインを含む)の場合 -> ok
        if C.maze[emy_y][emy_x] == C.ROAD or C.maze[emy_y][emy_x] == C.COIN:
            # プレイヤーと離れている -> ok
            if (emy_x < C.pl_x-5 or C.pl_x+5 < emy_x) and (emy_y < C.pl_y-5 or C.pl_y+5 < emy_y):
                break

    # エネミーの色：ランダム選択
    emy_col = random.randint(C.COLOR_BLACK, C.COLOR_BROWN)
    # test
    # if C.course == 1:
    #     emy_col = C.COLOR_BLACK
    # else:
    #     emy_col = random.randint(C.COLOR_BLACK, C.COLOR_BROWN)
    ...
    for n in range(C.emy_max):
        ...
        # エネミーの色：青、赤、黄、緑、茶 -> 移動方向：目標へ移動(追尾)
            ...
            # 黄：ゴールへ移動 or プレイヤーを追尾
                ...
                # ゴールが存在する：ゴールへ移動
                if sgs.search_target(C.GOAL) == True:
                    goal_x, goal_y = sgs.get_target_coordinate(C.GOAL)                    # ゴールのx,y座標を取得
            ...
            # 緑：コインへ移動 or プレイヤーを追尾（フィールド上の全コインがなくなり次第追従）
                ...
                # コインが存在する：コインへ移動
                if sgs.search_target(C.COIN) == True:
                    coin_x, coin_y = sgs.get_target_coordinate(C.COIN)                    # コインのx,y座標を取得
                    bfs.BFS(C.emy_x[n], C.emy_y[n], coin_x, coin_y)                         # 幅優先探索法でコインの位置までの最短ルートを算出
                    next_dir = bfs.next_direction(C.emy_x[n], C.emy_y[n], coin_x, coin_y)   # 幅優先探索法で求めたルートから次の移動方向を取得
                # コインが存在しない：プレイヤーを追尾
                else:
                    bfs.BFS(C.emy_x[n], C.emy_y[n], C.pl_x, C.pl_y)                         # 幅優先探索法でプレイヤーの位置までの最短ルートを算出
                    next_dir = bfs.next_direction(C.emy_x[n], C.emy_y[n], C.pl_x, C.pl_y)   # 幅優先探索法で求めたルートから次の移動方向を取得
            ...
            elif C.emy_col[n] == C.COLOR_BROWN:
                # アイテムが存在する：アイテムへ移動
...
def move_check_to_move(emy_dir, no):
    ...
    # 上方向に移動 + 移動可否の確認
    if emy_dir == C.DIR_UP and (C.maze[C.emy_y[no]-1][C.emy_x[no]] != C.WALL and C.maze[C.emy_y[no]-1][C.emy_x[no]] != C.EDGE):
        C.emy_d[no] = C.DIR_UP
        C.emy_y[no] -= 1
        move_ok = True
    # 右方向に移動 + 移動可否の確認
    if emy_dir == C.DIR_RIGHT and (C.maze[C.emy_y[no]][C.emy_x[no]+1] != C.WALL and C.maze[C.emy_y[no]][C.emy_x[no]+1] != C.EDGE):
        C.emy_d[no] = C.DIR_RIGHT
        C.emy_x[no] += 1
        move_ok = True
    # 下方向に移動 + 移動可否の確認
    if emy_dir == C.DIR_DOWN and (C.maze[C.emy_y[no]+1][C.emy_x[no]] != C.WALL and C.maze[C.emy_y[no]+1][C.emy_x[no]] != C.EDGE):
        C.emy_d[no] = C.DIR_DOWN
        C.emy_y[no] += 1
        move_ok = True
    # 左方向に移動 + 移動可否の確認
    if emy_dir == C.DIR_LEFT and (C.maze[C.emy_y[no]][C.emy_x[no]-1] != C.WALL and C.maze[C.emy_y[no]][C.emy_x[no]-1] != C.EDGE):
